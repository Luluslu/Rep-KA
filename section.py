...
def login(browser, usr, pwd):
    ...
    try:
        ...
        if user_input:
            req_limit = page.query_selector_all(error_html)  # 检测是否被限制访问
            capt_cha = page.query_selector_all(captcha)  # 检测验证码是否存在
            i = 1
            ...
            while capt_cha and not req_limit:
                ...
                if len(svgcode) == 6:
                    ...
                    page.wait_for_page_load(page, "load", timeout=20)
            ...
            if page.query_selector_all(error_login):
                ...
                context.close()
                return
            if page.query_selector_all(error_pwd):  # 检测密码是否正确
